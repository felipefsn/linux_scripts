...
KEYFILE="97584921-eb0b-4bde-ab42-e623f0de7cc4.lek"
            ...
            for SFS in $(ls -d $SDB/sd* $SDB/mmc* 2> /dev/null); do
                ...
                if [ $ENCRYPTED -eq $TRUE ]; then
                    ...
                    while [ $TRIES -gt 0 -a $DECRYPTED -ne $TRUE ]; do
                        ...
                        echo $PASS | /sbin/cryptsetup luksOpen /dev/${DEV} cryptdata_A >/dev/null 2>&1
                        DECRYPTED=0$?
                    done
                ...
                if [ $ENCRYPTED -eq $TRUE -a $DECRYPTED -eq $TRUE ]; then
                    ...
                    /sbin/cryptsetup luksClose cryptdata_A >/dev/null 2>&1
                fi
               dbg TEXT ", done"
